...
right; left. apply pgen_la in H3. destruct H3 as (A'' & M'' & D & D'' & s & t &u & h); decompose [and] h; clear h.
...
destruct H8 as (U0 & K & K' & T & T' & ? & -> & ? & _). exists (La C1 T'). econstructor. subst. apply H. trivial. apply H0. trivial.
assert (exists PP', Γ ⊢ P' ▹ PP' : Π(C'),E). destruct H13. destruct H13 as (P'' & ? & ? & ?). subst. exists P''; trivial.
destruct H13 as (U0 & K & K' & T & T' & ? & -> & ?  & _ ). exists (La C'1 T'). subst. econstructor. apply H1. trivial. apply  H7. trivial.
destruct H12 as (PP & ?). destruct H14 as (PP' & ?).
destruct (IHM4 Q' Γ Q1 C Q'1 C'  H2 H5 H10) as (RQ & ? & ? ). clear IHM4.
destruct (IHM1 P' Γ PP (Pi C D) PP' (Pi C' E) H3 H12 H14) as (RP & ? & ? ). clear IHM1. clear IHM2 IHM3.
...
rename H8 into HH8. assert (H8 : C::Γ ⊢ D ▹▹ Z : !t1). eapply typ_reds_relocate. apply HH8. apply  H0. clear HH8.
rename H13 into HH13. assert (H13 : C'::Γ ⊢ E ▹▹ Z : !t2). eapply typ_reds_relocate.  eapply conv_in_env_reds. apply HH13. eauto. apply  H7. clear HH13.
rename H21 into HH21. assert (H21 : Γ ⊢ C ▹▹ ZA : !s1). eapply typ_reds_relocate. apply HH21. apply H4. clear HH21.
rename H22 into HH22. assert (H22 : Γ ⊢ C' ▹▹ ZA : !s2). eapply typ_reds_relocate. apply HH22. apply H9. clear HH22.
clear d1 d2 d3 d4.
...
rename H13 into HH13. assert (H13 : C'::Γ ⊢ E ▹▹ Z : !t2). eapply typ_reds_relocate.  eapply conv_in_env_reds. apply HH13. eauto. apply  H7. clear HH13.
rename H21 into HH21. assert (H21 : Γ ⊢ C ▹▹ ZA : !s1). eapply typ_reds_relocate. apply HH21. apply H4. clear HH21.
rename H22 into HH22. assert (H22 : Γ ⊢ G ▹▹ ZA : !s2). eapply typ_reds_relocate. apply HH22. apply HH0. clear HH22.
clear d1 d2 d3 d4.
...
rename H8 into HH8. assert (H8 : G::Γ ⊢ D ▹▹ Z : !t1). eapply typ_reds_relocate. eapply conv_in_env_reds. apply HH8. eauto. eapply conv_in_env. apply  H0. eauto. clear HH8.
rename H13 into HH13. assert (H13 : C'::Γ ⊢ E ▹▹ Z : !t2). eapply typ_reds_relocate.  eapply conv_in_env_reds. apply HH13. eauto. apply  H7. clear HH13.
...
rename H8 into HH8. assert (H8 : G::Γ ⊢ D ▹▹ Z : !t1). eapply typ_reds_relocate. eapply conv_in_env_reds. apply HH8. eauto. eapply conv_in_env. apply  H0. eauto. clear HH8.
rename H13 into HH13. assert (H13 : F::Γ ⊢ E ▹▹ Z : !t2). eapply typ_reds_relocate.  eapply conv_in_env_reds. apply HH13. eauto. eapply conv_in_env. apply  H7. eauto. clear HH13.
rename H21 into HH21. assert (H21 : Γ ⊢ G ▹▹ ZA : !s1). eapply typ_reds_relocate. apply HH21. apply reds_refl_rt in HH2; apply HH2. clear HH21.
...
apply reds_typ_pcompat with (E [ ← Q']); trivial.  eapply reds_App. apply reds_typ_pcompat with (Π(C'),E); trivial. apply reds_typ_pcompat with C'; intuition. apply H22. apply H13.
  (** Phase 2 : RP -> λ **)
destruct H19 as (U & V & ? & ?). destruct (pgen_la Γ U V (λ[U],V) (Π(C),D)) as (U' & V' & K & K' & s & t & u & h).
apply red_refl_rt in H19; trivial. decompose [and] h; clear h. injection H25; intros; subst; clear H25.
destruct (pgen_la Γ U' V' (λ[U'],V') (Π(C'),E)) as (U'' & V'' & L & L'' & s' & t' & u' & h).
...
rename H36 into HH36. assert (H36: Γ ⊢ C ▹▹ ZA : !s1). eapply typ_reds_relocate. apply HH36. apply H4. clear HH36.
rename H8 into HH8. assert (H8 : U''::Γ ⊢ K ▹▹ ZB : !t). eapply typ_reds_relocate. apply HH8. apply H24. clear HH8.
rename H13 into HH13. assert (H13 : C::Γ ⊢ D ▹▹ ZB : !t1). eapply typ_reds_relocate. eapply conv_in_env_reds. apply HH13. eauto. apply H0. clear HH13.
clear a b c d.
...
rename H36 into HH36. assert (H36: Γ ⊢ C' ▹▹ ZA : !s2). eapply typ_reds_relocate. apply HH36. apply H9. clear HH36.
rename H8 into HH8. assert (H8 : U''::Γ ⊢ L ▹▹ ZB : !t'). eapply typ_reds_relocate. apply HH8. apply H29. clear HH8.
rename H13 into HH13. assert (H13 : C'::Γ ⊢ E ▹▹ ZB : !t2). eapply typ_reds_relocate. eapply conv_in_env_reds. apply HH13. eauto. apply H7. clear HH13.
clear a b c d.
...
rename H8 into HH8. assert (H8 : U''::Γ ⊢ L ▹▹ ZB : !t'). eapply typ_reds_relocate. apply HH8. apply H29. clear HH8.
rename H13 into HH13. assert (H13 : G::Γ ⊢ E ▹▹ ZB : !t2). eapply typ_reds_relocate. eapply conv_in_env_reds. apply HH13. eauto. eapply conv_in_env. apply H7. eauto. clear HH13.
clear a b c d.
assert (HEQ2: Γ ⊢ Π(C'),E ≡' Π(G),E). apply typ_peq_trans with (Π(G0),E). apply typ_peq_sym.
apply reds_to_conv with u2. eapply reds_Pi. apply HH1.  constructor. eapply conv_in_env. apply red_refl_lt in H7; apply H7. eauto. trivial.
apply reds_to_conv with u2. eapply reds_Pi. apply HH2.  constructor. eapply conv_in_env. apply red_refl_lt in H7; apply H7. eauto. trivial.
...
rename H8 into HH8. assert (H8 : U''::Γ ⊢ K ▹▹ ZB : !t). eapply typ_reds_relocate. apply HH8. apply H24. clear HH8.
rename H13 into HH13. assert (H13 : G::Γ ⊢ D ▹▹ ZB : !t1). eapply typ_reds_relocate.  eapply conv_in_env_reds. apply HH13. eauto. eapply conv_in_env. apply H0. eauto. clear HH13.
clear a b c d.
...
rename H8 into HH8. assert (H8 : U''::Γ ⊢ L ▹▹ ZB : !t'). eapply typ_reds_relocate. apply HH8. apply H29. clear HH8.
rename H13 into HH13. assert (H13 : F::Γ ⊢ E ▹▹ ZB : !t2). eapply typ_reds_relocate. eapply conv_in_env_reds. apply HH13. eauto. eapply conv_in_env. apply H7. eauto. clear HH13.
clear a b c d.
...
destruct (IHM1 N1 Γ M1' !s1 N1' !s2 H3 H4 H8) as (R1 &  ? & ?).
assert (Γ ⊢ M1 ≡' N1). apply typ_peq_trans with R1. apply reds_to_conv in H10; trivial. apply reds_to_conv in H12; intuition.
destruct (IHM2 N2 (M1::Γ) M2' !t1 N2' !t2 H2 H0 ) as (R2 & ? & ?).
eapply conv_in_env. apply H6. eauto. exists (Pi R1 R2); split.
...
(**)
destruct N; simpl in H; try discriminate. injection H; intros; subst; clear H.
apply pgen_la in H0. destruct H0 as (M1' & M2' & D & D' & s1 & t1 & u1 & h). decompose [and] h; clear h.
apply pgen_la in H1. destruct H1 as (N1' & N2' & D2 & D2' & s2 & t2 & u2 & h). decompose [and] h; clear h.
destruct (IHM1 N1 Γ M1' !s1 N1' !s2 H3 H4 H9) as (R1 & ? & ? ).
assert (Γ ⊢ M1 ≡' N1). apply typ_peq_trans with R1. apply reds_to_conv in H12; trivial. apply reds_to_conv in H14; intuition.
destruct (IHM2 N2 (M1::Γ) M2' D N2' D2 H2) as (R2 & ? & ? ); trivial.
eapply conv_in_env. apply H7. eauto. exists (La R1 R2); split.
...
Lemma ErasedTypeConversion : forall A B Γ A' s B' t, strip A = strip B -> Γ ⊢ A ▹ A' : !s -> Γ ⊢ B ▹ B' : !t ->
  Γ ⊢ A ≡' B.
intros.
destruct (ErasedTermConfluence A B Γ A' !s B' !t H H0 H1) as ( P & ?&  ?).
...
eapply ErasedTypeConversion. trivial. eapply conv_in_env. apply H7. apply IHΓ. trivial. apply wf_from_typ in H7; trivial.
apply wf_from_typ in H4; trivial. apply H4.  inversion H1; inversion H0; subst; clear H0 H1.
...
(**)
apply typ_la with B' s1 s2 s3; intuition. eapply H0. econstructor. eapply H.
